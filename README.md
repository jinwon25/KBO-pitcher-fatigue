...
flowchart LR
    ...
    C --> D[72.6 위험 경고 기준]
    D --> E[역전점 시뮬레이터]
    E --> F[개인 후속 고도화<br/>역할별·다음 등판·시간 외 검증]
```
...
`make check`는 데이터 계약, 핵심 결과, 역할별·시간 외 분석, 앱 기동, 그림 생성, 문서 링크, 대표 노트북 전체 실행을 검사합니다.
...
- 불펜은 GS 대신 gmLI·WPA·RE24 등 등판 맥락을 반영한 타깃으로 분리
- 투구 구속뿐 아니라 구속 저하, 구종별 구위, 제구 변화량을 시계열 피처로 확장
- 실제 부상·회복 기간 라벨과 엔트리 제외 사유를 연결
- 구단·시즌·투수 단위 계층 구조를 반영한 mixed-effects 또는 hierarchical model 적용
- 2025년 이후 외부 시즌에서 지수와 임계값 안정성 재검증
